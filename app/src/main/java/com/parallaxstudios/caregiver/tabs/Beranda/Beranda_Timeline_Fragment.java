package com.parallaxstudios.caregiver.tabs.Beranda;
...
import com.parallaxstudios.caregiver.adapter.BerandaTimelineAdapter;
...
public class Beranda_Timeline_Fragment extends Fragment {
	
	String urlPost;
    JSONObject jsonObjectDesignPosts;
    JSONArray jsonArrayDesignContent;
    ArrayList<BerandaTimeline> designs;
	...
	public Beranda_Timeline_Fragment(){}
	
	@Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
            Bundle savedInstanceState) {
 
        view = inflater.inflate(R.layout.fragment_beranda_timeline, container, false);
        ...
        protected void onPostExecute(String result) {
            ...
            if (designTitle.length != 0) {
                for (int i = postNumber - 1; i >= 0; i--) {
                    designs.add(new BerandaTimeline(designTitle[i], designExcerpt[i], designImage[i]));
                }
            }
            if (error) {
            ...
            recyclerViewAdapter = new BerandaTimelineAdapter(getActivity(), designs);

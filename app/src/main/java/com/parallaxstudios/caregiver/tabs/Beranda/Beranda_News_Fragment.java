...
import com.parallaxstudios.caregiver.adapter.BerandaNewsAdapter;
...
public class Beranda_News_Fragment extends Fragment {
	
	String urlPost;
    JSONObject jsonObjectDesignPosts;
    JSONArray jsonArrayDesignContent;
    ArrayList<BerandaNews> designs;
	...
	public Beranda_News_Fragment(){}
	
	@Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
            Bundle savedInstanceState) {
 
		view = inflater.inflate(R.layout.fragment_beranda_news, container, false);
        ...
        protected void onPostExecute(String result) {
            ...
            if (designTitle.length != 0) {
                for (int i = postNumber - 1; i >= 0; i--) {
                    designs.add(new BerandaNews(designTitle[i],designTitle[i], designExcerpt[i], designImageFull[i]));
                }
            }
            if (error) {
            ...
            recyclerViewAdapter = new BerandaNewsAdapter(getActivity(), designs);

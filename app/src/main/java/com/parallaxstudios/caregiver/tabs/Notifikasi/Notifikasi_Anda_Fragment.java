...
import com.parallaxstudios.caregiver.adapter.NotifikasiAndaAdapter;
import com.parallaxstudios.caregiver.model.NotifikasiAnda;
...
public class Notifikasi_Anda_Fragment extends Fragment {
	
	String urlPost;
    JSONObject jsonObjectDesignPosts;
    JSONArray jsonArrayDesignContent;
    ArrayList<NotifikasiAnda> designs;
	...
	public Notifikasi_Anda_Fragment(){}
	
	@Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
            Bundle savedInstanceState) {
 
        view = inflater.inflate(R.layout.fragment_notifikasi_anda, container, false);
        ...
        protected void onPostExecute(String result) {
            ...
            if (designTitle.length != 0) {
                for (int i = postNumber - 1; i >= 0; i--) {
                    designs.add(new NotifikasiAnda(designTitle[i], designExcerpt[i], designImage[i]));
                }
            }
            if (error) {
            ...
            recyclerViewAdapter = new NotifikasiAndaAdapter(getActivity(), designs);
